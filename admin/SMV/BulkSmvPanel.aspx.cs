...
public partial class admin_SMV_BulkSmvPanel : System.Web.UI.Page
{
    public string url = "http://web.kenpark-bangladesh.com/SMV/admin/SMV/BulkSmvPanel.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        costingSmvId = Convert.ToInt32(Request.QueryString["SmvValue"]);
        ...
        if (!IsPostBack)
        {
            ...
            LoadProductCategory();
			//LoadSeason();
		    //table = loadNewBulkSmvinformation(buyerId,styleNumber);
		   mainBody.Visible = false;
            ...
            if(costingSmvId!=0)
            {
                loadFromCostingPanel(costingSmvId);
                mainBody.Visible = true;
                buyerNameDropDownList.Enabled = false;
                //sampleStageDropDownList.Enabled = false;
                //fabricDropDownList.Enabled = false;
    ...
    protected void submitButton_Click(object sender, EventArgs e)
    {
        ...
        try
        {
			...
			//bulk.SeasonId = Convert.ToInt32(SeasonDropDownList.SelectedValue);
            ...
            bulk.CostingSmvId=Convert.ToInt32(valueAddedSmvHiddenField.Value);
            ...
            if(bulkSmvId==0)
            {
                ...
                bulk.UpdatedBy = 0;
                dt = bulkSmvBLL.InsertBulkSmvInfo(bulk);
    ...
    public string loadNewBulkSmvinformation(int buyerId, string styleNumber)
    {
        BulkSmvBLL smvBll = new BulkSmvBLL();

        try
        {
            bulkSmvTable = smvBll.LoadNewBulkSmvInfo(buyerId,styleNumber);
        }
        catch (Exception ex)
        {
    ...
    public void loadFromCostingPanel( int smvId)
    {
        int buyerIdForLoad = 0;
        try
        {
...

            {
                ...
                valueAddedSmvHiddenField.Value= Convert.ToString(dt.Rows[0]["SmvId"]);
				//SeasonDropDownList.SelectedValue= Convert.ToString(dt.Rows[0]["SeasonId"]);
                seasonTextBox.Text= Convert.ToString(dt.Rows[0]["SeasonId"]);
            }
        }
    ...
    {
        Response.Redirect("BulkSmvPanel.aspx");
    }
    ...
    protected void exlButton_Click(object sender, EventArgs e)
    {
        ...
        buyerId = Convert.ToInt32(searchDropDownList.SelectedValue);
        loadNewBulkSmvinformation(buyerId, styleNumber);
        tableRow.Append("<table class='table table-bordered' id='tableLoad'>" + loadNewBulkSmvinformation(buyerId, styleNumber) + "</table>");

        Response.Clear();
        Response.Buffer = true;
        Response.AddHeader("content-disposition", "attachment;filename=New_Bulk_Smv.xls");
        Response.Charset = "";
